...
public class PatternInspector : Editor
{
    ...
    protected SerializedProperty SpawnAnimationTime { get; set; }
    ...
    private const float BehaviourPaddingTop = 3;
    private const float BehaviourPaddingBottom = 7;
    private const float PopupSpacing = 2;
    ...
    protected virtual void DrawAnimationTimeField()
    {
        EditorGUILayout.PropertyField(SpawnAnimationTime);
    }
    ...
    protected virtual void DrawBehaviourHeader()
    {
        ...
        using (new EditorGUI.IndentLevelScope())
        {
            ...
            EditorGUI.LabelField(rect, "Behaviour");
        }
    }
    protected virtual void DrawBehaviourContent()
    {
        ...
        requiredHeight += BehaviourPaddingTop;
        requiredHeight += BehaviourPaddingBottom;
        requiredHeight += PopupSpacing;
        ...
        using (new EditorGUI.IndentLevelScope())
        {
            rect.y += BehaviourPaddingTop;
            rect.width -= 5;

            DrawBehaviourPopup(rect);

            rect.y += EditorGUIUtility.singleLineHeight + PopupSpacing;
    ...
    protected virtual void DrawPopup(Rect rect, out Rect popupRect, int elementIndex)
    {
        ...
        popupRect = new Rect(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight + PopupSpacing);
    ...
    protected virtual float GetElementHeight(SerializedProperty property)
    {
        ...
        return drawer.GetPropertyHeight(property, GUIContent.none) + EditorGUIUtility.singleLineHeight + PopupSpacing;
    }
    protected virtual void CreateSerializedProperties()
    {
        SpawnAnimationTime = serializedObject.FindProperty("spawnAnimationTime");
